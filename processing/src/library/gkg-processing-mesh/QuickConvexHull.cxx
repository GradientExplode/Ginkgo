...
//         - For each point, assign them to the first face for which they are
//           on the positive side of (so each point is assigned to at most
...
//                assigned to at most one of the newly created faces.
...
class VertexDataSource
{
  ...
  public:
    ...
    VertexDataSource( const std::vector< gkg::Vector3d< T > >& vec )
                    : m_ptr( ( const Vector3< T >* )&vec[ 0 ] ),
                      m_count( vec.size() )
    {
    }
...
class MeshBuilder
{
  ...
  public:
    ...
    std::array< size_t, 3 >
      getHalfEdgeIndicesOfFace( const Face& f ) const
    {
    ...
    //vectors.
    // Instead, they are marked as disabled which means that the indices
    // can be reused when we need to add new faces and half edges to the
    // mesh.
                  ...
                  const VertexDataSource< FloatType >& vertexData )
    {
      ...
      for ( const auto& face : builderObject.m_faces )
      {
        ...
        if ( !face.isDisabled() )
        {
          ...
          for ( const auto heIndex : heIndices )
          {
            ...
            const IndexType 
              vertexIndex = builderObject.m_halfEdges[ heIndex ].m_endVertex;
      ...
      for ( const auto& halfEdge : builderObject.m_halfEdges )
      {
        ...
        if ( !halfEdge.isDisabled() )
        {
          ...
          m_halfEdges.push_back( { static_cast< IndexType >(
                                                        halfEdge.m_endVertex ),
                ...
                bool CCW, bool useOriginalIndices )
    {
      ...
      while ( faceStack.size() )
      {
        ...
        else
        {
          ...
          auto halfEdges = mesh.getHalfEdgeIndicesOfFace( 
                                                         mesh.m_faces[ top ] );
          size_t adjacent[] =
               { mesh.m_halfEdges[ 
                   mesh.m_halfEdges[ halfEdges[ 0 ] ].m_opp ].m_face,
                 mesh.m_halfEdges[
                   mesh.m_halfEdges[ halfEdges[ 1 ] ].m_opp ].m_face,
                 mesh.m_halfEdges[
                   mesh.m_halfEdges[ halfEdges[ 2 ] ].m_opp ].m_face };
          for ( auto a : adjacent )
          {
...
class QuickHull
{
  ...
  public:
    ...
    // Params:
    //   pointCloud: a vector of Ginkgo Vector3d points
    ...
    // the original point cloud. If this is false, then we generate a new 
    ...
    ConvexHull< FloatType > 
      getConvexHull( const std::vector< gkg::Vector3d< FloatType > >&
                                                                    pointCloud,
                     bool CCW,
    ...
    // positive of it (for a point cloud with scale 1)
    ConvexHull< FloatType > 
      getConvexHull( const std::vector< Vector3< FloatType > >& pointCloud,
                     bool CCW,
                     bool useOriginalIndices,
                     FloatType eps = defaultEps< FloatType >() );
    ...
    ConvexHull< FloatType >
      getConvexHull( const Vector3< FloatType >* vertexData,
                     size_t vertexCount,
                     bool CCW,
                     bool useOriginalIndices,
                     FloatType eps = defaultEps< FloatType >() );
    ...
    // vertices that are part of the convex hull.
    //   eps: minimum distance to a plane to consider a point being on 
    // positive side of it (for a point cloud with scale 1)
    ConvexHull< FloatType >
      getConvexHull( const FloatType* vertexData,
                     size_t vertexCount,
                     bool CCW,
                     bool useOriginalIndices,
                     FloatType eps = defaultEps< FloatType >() );
    ...
    //  the vertex data resides in memory
    // in the following format: x_0,y_0,z_0,x_1,y_1,z_1,...
    // Params:
    ...
    HalfEdgeMesh< FloatType, size_t >
      getConvexHullAsMesh( const FloatType* vertexData,
                           size_t vertexCount,
                           bool CCW,
                           FloatType eps = defaultEps< FloatType >() );
  ...
  private:
    ...
    // the QuickHull iteration proceeds. m_extremeValues must be properly set
    ...
    // Each face contains a unique pointer to a vector of indices. However, 
    // many - often most - faces do not have any points on the positive
    // side of them especially at the the end of the iteration. When a face is
    // removed from the mesh, its associated point vector, if such
    // exists, is moved to the index vector pool, and when we need to add new 
    // faces with points on the positive side to the mesh,
    // we reuse these vectors. This reduces the amount of std::vectors we have
    ...
    void buildMesh( const VertexDataSource< FloatType >& pointCloud,
                    bool CCW,
                    bool useOriginalIndices,
                    FloatType eps );

    // The public getConvexHull functions will setup a VertexDataSource object
    // and call this
    ConvexHull< FloatType >
      getConvexHull( const VertexDataSource< FloatType >& pointCloud,
                     bool CCW,
                     bool useOriginalIndices,
                     FloatType eps );

    FloatType m_epsilon;
    FloatType m_epsilonSquared;
    FloatType m_scale;
    bool m_planar;
...
template < typename T >
ConvexHull< T >
QuickHull< T >::getConvexHull( const std::vector< Vector3< T > >& pointCloud,
                               bool CCW,
                               bool useOriginalIndices,
                               T epsilon )
{
...
template < typename T >
ConvexHull< T >
QuickHull< T >::getConvexHull( const std::vector< gkg::Vector3d< T > >& 
                                                                    pointCloud,
                               bool CCW,
                               bool useOriginalIndices,
                               T epsilon )
...
template < typename T >
ConvexHull< T >
QuickHull< T >::getConvexHull( const Vector3< T >* vertexData,
                               size_t vertexCount,
                               bool CCW,
                               bool useOriginalIndices,
                               T epsilon )
...
template < typename T >
ConvexHull< T >
QuickHull< T >::getConvexHull( const T* vertexData, 
                               size_t vertexCount,
                               bool CCW,
                               bool useOriginalIndices,
                               T epsilon )
...
template < typename T >
ConvexHull< T >
QuickHull< T >::getConvexHull( const VertexDataSource< T >& pointCloud,
                               bool CCW,
                               bool useOriginalIndices,
                               T epsilon )
{
  ...
  while ( !m_faceList.empty() )
  {
    ...
    while ( m_possiblyVisibleFaces.size() )
    {
          ...
          for ( auto heIndex : m_mesh.getHalfEdgeIndicesOfFace( pvf ) )
          {
                 ...
                 faceData.m_enteredFromHalfEdge )
            {
              ...
              m_possiblyVisibleFaces.emplace_back(
                      m_mesh.m_halfEdges[
                        m_mesh.m_halfEdges[ heIndex ].m_opp ].m_face,heIndex );
      ...
      const auto halfEdges = m_mesh.getHalfEdgeIndicesOfFace(
                              m_mesh.m_faces[
                                m_mesh.m_halfEdges[ 
                                  faceData.m_enteredFromHalfEdge ].m_face ] );
      const std::int8_t 
        ind = ( halfEdges[ 0 ] == faceData.m_enteredFromHalfEdge ) ?
              0 : ( halfEdges[ 1 ] == faceData.m_enteredFromHalfEdge ? 1 : 2 );
      m_mesh.m_faces[ 
        m_mesh.m_halfEdges[
          faceData.m_enteredFromHalfEdge ].m_face ].
    ...
    // be marked as disabled. Their data slots will be reused.
    // The faces will be disabled as well, but we need to remember the points
    // that were on the positive side of them - therefore
    ...
    for ( auto faceIndex : m_visibleFaces )
    {
      ...
      // positive side of it. We need to assign those points
    ...
    for ( size_t i = 0; i < horizonEdgeCount; i++ )
    {
      ...
      auto horizonEdgeVertexIndices = m_mesh.getVertexIndicesOfHalfEdge(
      ...
      const Vector3< T > 
        planeNormal = mathutils::getTriangleNormal( m_vertexData[ A ],
                                                    m_vertexData[ B ],
                                                    activePoint );
...
bool QuickHull< T >::reorderHorizonEdges( std::vector< size_t >& horizonEdges )
{
  ...
  for ( size_t i = 0; i < horizonEdgeCount - 1; i++ )
  {
    ...
    const size_t
      endVertex = m_mesh.m_halfEdges[ horizonEdges[ i ] ].m_endVertex;
    bool foundNext = false;
    for ( size_t j = i + 1; j < horizonEdgeCount; j++ )
    {

      const size_t 
        beginVertex = m_mesh.m_halfEdges[
                        m_mesh.m_halfEdges[
                          horizonEdges[ j ] ].m_opp ].m_endVertex;
  ...
  assert( m_mesh.m_halfEdges[ 
            horizonEdges[ horizonEdges.size() - 1 ] ].m_endVertex ==
          m_mesh.m_halfEdges[
            m_mesh.m_halfEdges[ horizonEdges[ 0 ] ].m_opp ].m_endVertex );
  return true;
...
void QuickHull< T >::setupInitialTetrahedron()
{
  ...
  if ( vertexCount <= 4 )
  {
    ...
    const Vector3< T >
      N = mathutils::getTriangleNormal( m_vertexData[ v[ 0 ] ],
                                        m_vertexData[ v[ 1 ] ],
                                        m_vertexData[ v[ 2 ] ]);
  ...
  if ( maxD == m_epsilonSquared )
  {
    ...
    // R^3: convex hull has no volume => return a thin triangle
    // Pick any point other than selectedPoints.first and selectedPoints.second
    //  as the third point of the triangle
    auto it = std::find_if( m_vertexData.begin(),
                            m_vertexData.end(),
                            [&]( const vec3& ve )
                            {
    ...
    it = std::find_if( m_vertexData.begin(),
                       m_vertexData.end(),
                       [&]( const vec3& ve )
                       {
    ...
    const size_t fourthPoint = (it == m_vertexData.end() ) ?
                               selectedPoints.first :
                               std::distance( m_vertexData.begin(), it );
    return m_mesh.setup( selectedPoints.first,
  ...
  std::array< size_t, 3 >
    baseTriangle{ selectedPoints.first, selectedPoints.second, maxI };
  const Vector3< T > 
    baseTriangleVertices[]= { m_vertexData[ baseTriangle[ 0 ] ],
  ...
  maxI = 0;
  const Vector3<T> 
    N = mathutils::getTriangleNormal( baseTriangleVertices[ 0 ],
...
gkg::QuickConvexHull::QuickConvexHull()
{
}
...
// 	ConvexHull< float >
//        hull = qh.getConvexHull(pointCloud, CCW = true, useOriginal=false);
...
void gkg::QuickConvexHull::getConvexHull(
                     ...
                     std::list< gkg::Polygon< 3U > >& polygons ) const
{
  ...
  try
  {
    ...
    quickhull::ConvexHull< float >
      convexHull = quickHull.getConvexHull( points, true, false );

    const quickhull::VertexDataSource< float >& 
      vertexBuffer = convexHull.getVertexBuffer();
    
    size_t vertexCount = vertexBuffer.size();
    size_t v = 0U;
    for ( v = 0U; v < vertexCount; v++ )
    {

      vertices.push_back( gkg::Vector3d< float >( vertexBuffer[ v ].x,
                                                  vertexBuffer[ v ].y,
                                                  vertexBuffer[ v ].z ) );
    ...
    const std::vector< size_t >& 
      polygonBuffer = convexHull.getIndexBuffer();

    size_t polygonCount = polygonBuffer.size() / 3U;
    size_t p = 0;
    for ( p = 0U; p < polygonCount; p++ )
    {

      polygons.push_back( gkg::Polygon< 3U >( polygonBuffer[ p * 3 ],
                                              polygonBuffer[ p * 3 + 1 ],
                                              polygonBuffer[ p * 3 + 2 ] ) );                                              
  ...
  GKG_CATCH( "void gkg::QuickConvexHull::getConvexHull( "
             "const std::vector< Vector3d< float > >& points, "
             "std::list< gkg::Vector3d< float > >& vertices, "
             "std::list< gkg::Polygon< 3U > >& polygons ) const" );
