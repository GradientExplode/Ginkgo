...
#include <gkg-mri-reconstruction-io/GehcSignaGenesis2Header.h>
...
#define GENESIS2_IMAGE_MAGIC 0x494d4746
...
template < class T >
gkg::GehcSignaGenesis2DiskFormat< T >::GehcSignaGenesis2DiskFormat()
...
template < class T >
std::string gkg::GehcSignaGenesis2DiskFormat< T >::getName() const
{

  return "gehc-signa-genesis2";

}


template < class T >
void gkg::GehcSignaGenesis2DiskFormat< T >::readHeader(
                                             ...
                                             gkg::HeaderedObject& object ) const
{
  ...
  try
  {
    ...
    bool bswap = false;
    float revision = -1.0;
    uint32_t pixHdrOffset = -1;
    uint32_t suiteDataTypeOffset = -1;
    uint32_t examDataTypeOffset = -1;
    uint32_t seriesDataTypeOffset = -1;
    uint32_t mrImageDataTypeOffset = -1;
    getBSwapAndRevisionAndOffsets( name,
                                   bswap,
                                   revision,
                                   pixHdrOffset,
                                   suiteDataTypeOffset,
                                   examDataTypeOffset,
                                   seriesDataTypeOffset,
                                   mrImageDataTypeOffset );
    ...
    std::map< std::string,
              typename gkg::GehcSignaGenesis2DiskFormat< T >::SliceInfo >
      nameToSliceInfoLut;
    ...
    while ( f != fe )
    {
      ...
      typename gkg::GehcSignaGenesis2DiskFormat< T >::SliceInfo sliceInfo;
    ...
    typename gkg::GehcSignaGenesis2DiskFormat< T >::SliceInfo dummySliceInfo;
    ...
    int32_t imgDepth;
    object.getHeader().getAttribute( "genesis2_header.rdb_hdr_pixhdr.img_depth",
                                     imgDepth );
    if ( object.getHeader().hasAttribute( "item_type" ) )
    {
    ...
    object.addSemantic( "__gehc_signa_genesis2__",
                        "ctr_R",
                        gkg::Semantic(
    ...
    object.getHeader().getAttribute( "genesis2_header.rdb_hdr_image.imatrix_X",
                                     imatrix_X );
    object.getHeader().getAttribute( "genesis2_header.rdb_hdr_image.imatrix_Y",
                                     imatrix_Y );
    object.getHeader().getAttribute( "genesis2_header.rdb_hdr_image.pixsize_X",
                                     pixsize_X );
    object.getHeader().getAttribute( "genesis2_header.rdb_hdr_image.pixsize_Y",
                                     pixsize_Y );
    object.getHeader().getAttribute( "genesis2_header.rdb_hdr_image.slthick",
                                     slthick );
    object.getHeader().getAttribute("genesis2_header.rdb_hdr_image.scanspacing",
                                     scanspacing );
    ...
    object.addSemantic( "__gehc_signa_genesis2__", "resolutionX",
                        gkg::Semantic( gkg::TypeOf< double >::getName() ) );
    object.addSemantic( "__gehc_signa_genesis2__", "resolutionY",
                        gkg::Semantic( gkg::TypeOf< double >::getName() ) );
    object.addSemantic( "__gehc_signa_genesis2__", "resolutionZ",
                        gkg::Semantic( gkg::TypeOf< double >::getName() ) );
    object.addSemantic( "__gehc_signa_genesis2__", "resolutionT",
                        gkg::Semantic( gkg::TypeOf< double >::getName() ) );
...
template < class T >
void gkg::GehcSignaGenesis2DiskFormat< T >::read(
                                                const std::string& name,
...
template < class T >
inline
void gkg::GehcSignaGenesis2DiskFormat< T >::preparePartialBinaryRead(
                                             const std::string& name,
                                             gkg::HeaderedObject& proxy ) const
{
  ...
  try
  {
    ...
    if ( &dynamic_cast< gkg::VolumeProxy< T >& >( proxy ) )
    {
      ...
      volumeProxy.getHeader().addAttribute( "proxyPixHdrOffset",
                                            pixHdrOffset );
      volumeProxy.getHeader().addAttribute( "proxySuiteDataTypeOffset",
                                            suiteDataTypeOffset );
      volumeProxy.getHeader().addAttribute( "proxyExamDataTypeOffset",
                                            examDataTypeOffset );
      volumeProxy.getHeader().addAttribute( "proxySeriesDataTypeOffset",
                                            seriesDataTypeOffset );
      volumeProxy.getHeader().addAttribute( "proxyMrImageDataTypeOffset",
                                            mrImageDataTypeOffset );
  ...
  GKG_CATCH( "template < class T > void gkg::GehcSignaGenesis2DiskFormat< T >::"
...
template < class T >
void gkg::GehcSignaGenesis2DiskFormat< T >::partialBinaryRead(
...
template < class T >
bool gkg::GehcSignaGenesis2DiskFormat< T >::hasReader() const
{
...
template < class T >
void gkg::GehcSignaGenesis2DiskFormat< T >::getBSwapAndRevisionAndOffsets( 
                                     const std::string& name,
                                     bool& bswap,
                                     float& revision,
                                     ...
                                     uint32_t& mrImageDataTypeOffset ) const
{
  ...
  try
  {
    ...
    // checking magic number for byte swapping
    int32_t imgMagic;
    bswap = false;
    gkg::ItemReader< int32_t >* itemR_INT32 =
      &gkg::DefaultItemIOFactory< int32_t >::getInstance().getReader( false,
                                                                      false );
    itemR_INT32->read( is, imgMagic );
    if ( imgMagic == GENESIS2_IMAGE_MAGIC )
    {
    ...
    else
    {
      ...
      else
      {
        ...
        throw std::runtime_error( "not a Genesis 2 file" );

      }

    }

    // getting pix header, suite, exam, series, mr image offset(s)

    int32_t tmp = -1;

    pixHdrOffset = 0U;

    is.seekg( 124U, gkg::IosBase::beg );
    itemR_INT32->read( is, tmp );
    suiteDataTypeOffset = ( uint32_t )tmp;

    is.seekg( 132U, gkg::IosBase::beg );
    itemR_INT32->read( is, tmp );
    examDataTypeOffset = ( uint32_t )tmp;

    is.seekg( 140U, gkg::IosBase::beg );
    itemR_INT32->read( is, tmp );
    seriesDataTypeOffset = ( uint32_t )tmp;

    is.seekg( 148U, gkg::IosBase::beg );
    itemR_INT32->read( is, tmp );
    mrImageDataTypeOffset = ( uint32_t )tmp;
    ...
    revision = -1.0;
    is.seekg( suiteDataTypeOffset + 20U, gkg::IosBase::beg );
  ...
  GKG_CATCH( "template < class T > "
             "void gkg::GehcSignaGenesis2DiskFormat< T >::"
             "getBSwapAndRevisionAndOffsets( "
...
template < class T >
void gkg::GehcSignaGenesis2DiskFormat< T >::readCoreHeader(
           const std::string& name,
           ...
           uint32_t mrImageDataTypeOffset ) const
{
  ...
  try
  {
    ...
    readPixHdr( is, "genesis2_header.rdb_hdr_pixhdr",
                object, sliceInfo, bswap, revision,
                pixHdrOffset );
    readSuiteDataType( is, "genesis2_header.rdb_hdr_suite",
                       object, bswap, revision,
                       suiteDataTypeOffset );
    readExamDataType( is, "genesis2_header.rdb_hdr_exam",
                      object, bswap, revision,
                      examDataTypeOffset );
    readSeriesDataType( is, "genesis2_header.rdb_hdr_series",
                        object, bswap, revision,
                        seriesDataTypeOffset );
    readMrImageDataType( is, "genesis2_header.rdb_hdr_image",
                         object, sliceInfo, bswap, revision,
                         mrImageDataTypeOffset );
...
template < class T >
void gkg::GehcSignaGenesis2DiskFormat< T >::readPartialBinaryCore(
                                 ...
                                 float /* revision */ ) const
{
  ...
  try
  {
    ...
    int32_t genesis2HeaderLength;
    volumeProxy.getHeader().getAttribute( "genesis2_header_length",
                                          genesis2HeaderLength );
...
template < class T >
void gkg::GehcSignaGenesis2DiskFormat< T >::readPixHdr(
           std::istream& is, 
           ...
           uint32_t pixHdrOffset  ) const
{
  ...
  try
  {
    ...
    is.seekg( pixHdrOffset, gkg::IosBase::beg );
    ...
    readItem( is, itemR_INT32, prefix, "img_p_suite", object );
    readItem( is, itemR_INT32, prefix, "img_l_suite", object );
    readItem( is, itemR_INT32, prefix, "img_p_exam", object );
    readItem( is, itemR_INT32, prefix, "img_l_exam", object );
    readItem( is, itemR_INT32, prefix, "img_p_series", object );
    readItem( is, itemR_INT32, prefix, "img_l_series", object );
    readItem( is, itemR_INT32, prefix, "img_p_image", object );
    readItem( is, itemR_INT32, prefix, "img_l_image", object );
    ...
    object.getHeader().getAttribute( prefix + ".img_hdr_length",
                                     pixhdr_length );
    object.addSemantic( "__gehc_signa_genesis2__",
                        "genesis2_header_length",
                        gkg::Semantic( gkg::TypeOf< int32_t >::getName() ) );
    object.getHeader().addAttribute( "genesis2_header_length", 
                                     pixhdr_length );
...
template < class T >
void gkg::GehcSignaGenesis2DiskFormat< T >::readSuiteDataType(
                                        std::istream& is, 
                                        const std::string& prefix,
                                        gkg::HeaderedObject& object,
                                        bool bswap,
                                        ...
                                        uint32_t suiteDataTypeOffset ) const
{
  ...
  try
  {
    ...
    is.seekg( suiteDataTypeOffset, gkg::IosBase::beg );
...
template < class T >
void gkg::GehcSignaGenesis2DiskFormat< T >::readExamDataType(
                                        std::istream& is, 
                                        const std::string& prefix,
                                        gkg::HeaderedObject& object,
                                        bool bswap,
                                        ...
                                        uint32_t examDataTypeOffset ) const
{
  ...
  try
  {
    ...
    is.seekg( examDataTypeOffset, gkg::IosBase::beg );
...
template < class T >
void gkg::GehcSignaGenesis2DiskFormat< T >::readSeriesDataType(
                                      std::istream& is, 
                                      const std::string& prefix,
                                      gkg::HeaderedObject& object,
                                      bool bswap,
                                      ...
                                      uint32_t seriesDataTypeOffset ) const
{
  ...
  try
  {
    ...
    is.seekg( seriesDataTypeOffset, gkg::IosBase::beg );
...
template < class T >
void gkg::GehcSignaGenesis2DiskFormat< T >::readMrImageDataType(
           std::istream& is, 
           const std::string& prefix,
           gkg::HeaderedObject& object,
           typename gkg::GehcSignaGenesis2DiskFormat< T >::SliceInfo& sliceInfo,
           ...
           uint32_t mrImageDataTypeOffset ) const
{
  ...
  try
  {
    ...
    is.seekg( mrImageDataTypeOffset, gkg::IosBase::beg );
...
void gkg::GehcSignaGenesis2DiskFormat< T >::readBaseItem(
                                       std::istream& is, 
...
template < class T > template < class U >
void gkg::GehcSignaGenesis2DiskFormat< T >::readItem(
                                             std::istream& is, 
                                             const gkg::ItemReader< U >& itemR,
                                             const std::string& prefix,
                                             const std::string& name,
...
template < class T >
void gkg::GehcSignaGenesis2DiskFormat< T >::readStringItem(
                                         std::istream& is, 
...
template < class T >
void gkg::GehcSignaGenesis2DiskFormat< T >::readVarTypeItem(
                            std::istream& is, 
...
template class gkg::GehcSignaGenesis2DiskFormat< uint8_t >;
template class gkg::GehcSignaGenesis2DiskFormat< int16_t >;


//
// registrating standard GEHC Signa Genesis 2 disk format(s) for "Volume" 
// object factory
//
...
static bool initialize()
{
  ...
  try
  {
    ...
    gkg::DiskFormatFactory< gkg::Volume< uint8_t > 
         ...
         &gkg::GehcSignaGenesis2DiskFormat< uint8_t >::getInstance() );
    gkg::DiskFormatFactory< gkg::Volume< int16_t > 
                           >::getInstance().registerDiskFormat(
         gkg::GehcSignaGenesis2DiskFormat< int16_t >::getInstance().getName(),
         &gkg::GehcSignaGenesis2DiskFormat< int16_t >::getInstance() );

    return true;

  }
  GKG_CATCH( "GEHC Signa Genesis 2 disk format registration" );
