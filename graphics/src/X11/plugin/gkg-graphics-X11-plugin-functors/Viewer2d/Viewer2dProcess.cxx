...
#define DEFAULT_SPAN_X    256
#define DEFAULT_SPAN_Y    256
                                 ...
                                 const std::string& )
{
  ...
  try
  {
    ...
    float assymetry = ( float )volumeModel->getSpanX() /
                      ( float )volumeModel->getSpanY();

    gkg::MonoGlyph*
      layoutFixed = layoutFactory.createFixed(
                         viewer2d,
                         DEFAULT_SPAN_X *
                         viewer2dProcess.getScale(),
                         ( DEFAULT_SPAN_Y / assymetry ) *
                         viewer2dProcess.getScale() );
