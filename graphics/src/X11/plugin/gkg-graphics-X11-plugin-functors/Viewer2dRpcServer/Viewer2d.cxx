...
static gkg::FloatColorIntensity rainbow2[ 256 ][ 3 ] =
  {
    { 0.000, 0.000, 0.000 },
    { 0.016, 0.000, 0.012 },
    { 0.035, 0.000, 0.027 },
...
gkg::Viewer2d::Viewer2d( bool doRgb, bool rainbowPalette )
              : gkg::InputHandler( NULL,
                                   gkg::Session::getInstance().
                                              getDefaultDisplay()->getStyle() ),
                _rainbowPalette( rainbowPalette ),
                _doRgb( doRgb )
{
...
gkg::Volume< gkg::RGBComponent >& gkg::Viewer2d::getRGBVolume()
{
...
void gkg::Viewer2d::reset( bool doRedraw )
{
  ...
  try
  {
    ...
    _fusionPercentage = _doRgb ? 30 : 50;
    _deltaFusion = 0;
    _first = true;
    _multiVolume = false;
    _repetitionCount = 0;
...
void gkg::Viewer2d::update()
{
  ...
  try
  {
    ...
    if ( _doRgb )
    {
      ...
      //_multiVolume = _volumeRGB.hasSameSize( _volumeRGB );
      _multiVolume = ( _volumeRGB.getSizeXYZT() > 1 ) ? true : false;
    ...
    else
    {
      ...
      _multiVolume = ( _volumeActivation.getSizeXYZT() > 1 ) ? true : false;
    ...
    if ( _multiVolume && _repetitionCount < 15 )
    {
...
void gkg::Viewer2d::fillRaster( int32_t slice, int32_t rank )
{
  ...
  try
  {
    ...
    double ry_multi = 1.0;
    double rz_multi = 1.0;
    if ( _volumeAverage.getHeader().hasAttribute( "resolutionX" ) &&
         _volumeAverage.getHeader().hasAttribute( "resolutionY" ) &&
	 _volumeAverage.getHeader().hasAttribute( "resolutionZ" ) )
    {
    
      _volumeAverage.getHeader().getAttribute( "resolutionX", rx_avg );
      _volumeAverage.getHeader().getAttribute( "resolutionY", ry_avg );
      _volumeAverage.getHeader().getAttribute( "resolutionZ", rz_avg );
    ...
    if ( _multiVolume )
    {
      ...
      if ( _doRgb )
      {
        ...
        if ( _volumeRGB.getHeader().hasAttribute( "resolutionX" ) &&
             _volumeRGB.getHeader().hasAttribute( "resolutionY" ) &&
	     _volumeRGB.getHeader().hasAttribute( "resolutionZ" ) )
    	{
    
    	  _volumeRGB.getHeader().getAttribute( "resolutionX", rx_multi );
    	  _volumeRGB.getHeader().getAttribute( "resolutionY", ry_multi );
    	  _volumeRGB.getHeader().getAttribute( "resolutionZ", rz_multi );
      ...
      else
      {
        ...
        if ( _volumeActivation.getHeader().hasAttribute( "resolutionX" ) &&
             _volumeActivation.getHeader().hasAttribute( "resolutionY" ) &&
	     _volumeActivation.getHeader().hasAttribute( "resolutionZ" ) )
    	{
    
    	  _volumeActivation.getHeader().getAttribute( "resolutionX", rx_multi );
    	  _volumeActivation.getHeader().getAttribute( "resolutionY", ry_multi );
    	  _volumeActivation.getHeader().getAttribute( "resolutionZ", rz_multi );
        ...
        if ( _multiVolume )
	{
          ...
          if ( _doRgb )
          {
            ...
            int32_t x1 = (int32_t)( (double)x * rx_avg / rx_multi );
            int32_t y1 = (int32_t)( (double)y * ry_avg / ry_multi );
            int32_t z1 = (int32_t)( (double)slice * rz_avg / rz_multi );

            gkg::RGBComponent rgb = _volumeRGB( x1, y1, z1, rank );
            r_cor = (float)rgb.r / 255.0;
            g_cor = (float)rgb.g / 255.0;
            b_cor = (float)rgb.b / 255.0;
            a_cor = 1.0;
          ...
          else
          {
            ...
            threshold( _volumeActivation( x1, y1, z1, rank ), 
	               r_cor, g_cor, b_cor, a_cor,
		       _lowerThresholdActivation, _upperThresholdActivation,
		       _rainbowPalette ? rainbow2 : red_temperature );
