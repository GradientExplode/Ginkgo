...
double gkg::PolarHarmonicsCache::getTmn( int32_t m, int32_t n,
    ...
    if ( ( m + n ) % 2 == 0 )
    {
      ...
      for ( r = 0; r <= m; r += 2 )
      {
        ...
        for ( s = 0; s <= n; s += 2 )
        {
          ...
          t += ( factor *
                 std::pow( ( double )( 1.0 / ( u * u ) + 1.0 / ( v * v ) ),
                                  ( double )( -0.5 * ( m + n - r - s + 1 ) ) ) *
                 ( ( ( ( r + s ) / 2 ) % 2 == 1 ) ? -1.0 : +1.0 ) *
                 std::pow( 2.0, ( double )( 0.5 * ( m + n - r - s ) ) ) *
                 this->getDoubleFactorialFunction( ( double )( m + n - r - s - 1 ) ) ) /
               ( std::pow( ( double )u, ( double )( m - r ) ) *
                 std::pow( ( double )v, ( double )( n - s ) ) * 
                 ( double )_factory->getFactorialFunction( ( double )( m - r ) ) *
                 ( double )_factory->getFactorialFunction( ( double )( n - s ) ) *
                 this->getDoubleFactorialFunction( ( double )r ) *
                 this->getDoubleFactorialFunction( ( double )s ) );
        }
  ...
  try
  {
    ...
    return ( 1.0 / ( ( float )std::sqrt( std::pow( 2, order + 1 ) *
           ...
           ( float )std::exp( -std::pow(
                             1e-6 * rotatedDisplacements[ displacementIndex ].x,
                                2 ) /
  ...
  try
  {
    ...
    return ( 1.0 / ( std::sqrt( std::pow( 2, order + 1 ) *
                                ...
                                _factory->getFactorialFunction(
                                                   ( double )order ) ) *
                     scaling ) ) *
           std::exp( -std::pow( 1e-6 * rotatedDisplacements[ displacementIndex ].y,
                                2 ) /
                     ( 2 * std::pow( scaling, 2 ) ) ) *
           _factory->getHermitePolynomialFunction(
                            order,
                            1e-6 * rotatedDisplacements[ displacementIndex ].y /
                            scaling );
  ...
  try
  {
    ...
    return ( 1.0 / ( std::sqrt( std::pow( 2, order + 1 ) *
                                ...
                                _factory->getFactorialFunction(
                                                   ( double )order ) ) *
                     scaling ) ) *
           std::exp( -std::pow( 1e-6 * rotatedDisplacements[ displacementIndex ].z,
                                2 ) /
                     ( 2 * std::pow( scaling, 2 ) ) ) *
           _factory->getHermitePolynomialFunction(
                            order,
                            1e-6 * rotatedDisplacements[ displacementIndex ].z /
                            scaling );
...
double gkg::PolarHarmonicsCache::getDoubleFactorialFunction( double r ) const
{
