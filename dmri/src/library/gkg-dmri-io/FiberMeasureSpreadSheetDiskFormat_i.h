#ifndef _gkg_dmri_io_FiberMeasureSpreadSheetDiskFormat_i_h_
#define _gkg_dmri_io_FiberMeasureSpreadSheetDiskFormat_i_h_


#include <gkg-dmri-io/FiberMeasureSpreadSheetDiskFormat.h>
...
inline
gkg::FiberMeasureSpreadSheetDiskFormat< L, Compare >::
                                             FiberMeasureSpreadSheetDiskFormat()
                                                     : gkg::TypedDiskFormat<
                                  gkg::FiberMeasureSpreadSheet< L, Compare > >()
{

  this->_extensions.insert( ".fibermeasure_spreadsheet" );
...
inline
std::string 
gkg::FiberMeasureSpreadSheetDiskFormat< L, Compare >::getName() const
{

  return "fibermeasure_spreadsheet";
...
template < class L, class Compare >
inline
void gkg::FiberMeasureSpreadSheetDiskFormat< L, Compare >::readHeader(
                                             ...
                                             gkg::HeaderedObject& object ) const
{
  ...
  try
  {
    ...
    if ( objectType != "FiberMeasureSpreadSheet" )
    {
  ...
  GKG_CATCH( "template < class L, class Compare > "
             "inline "
             "void gkg::FiberMeasureSpreadSheetDiskFormat< L, Compare >::"
             "readHeader( "
...
template < class L, class Compare >
inline
void gkg::FiberMeasureSpreadSheetDiskFormat< L, Compare >::writeHeader(
                                       const std::string& name,
...
template < class L, class Compare >
inline
void gkg::FiberMeasureSpreadSheetDiskFormat< L, Compare >::read(
                      const std::string& name,
                      gkg::FiberMeasureSpreadSheet< L, Compare >& object ) const
{
  ...
  try
  {
    ...
    std::ifstream
      is( ( this->getNameWithoutExtension( name ) +
            ".fibermeasure_spreadsheet" ).c_str() );
    if ( !is )
    {

      throw gkg::FileNotFoundException(
        this->getNameWithoutExtension( name ) + ".fibermeasure_spreadsheet" );
    ...
    while ( !is.eof() )
    {
      ...
      for ( s1 = 0; s1 < size1; s1++ )
      {
        ...
        if ( s1 )
        {
          ...
          is >> labelForCheck;
          is >> fiberIndexForCheck;
          is >> measureNameForCheck;
          is >> size1ForCheck;
          is >> size2ForCheck;
          if ( ( labelForCheck != label ) ||
               ( fiberIndexForCheck != fiberIndex ) ||
      ...
      object.addMeasure( label, fiberIndex, measureName, measureMatrix );
...
template < class L, class Compare >
inline
void gkg::FiberMeasureSpreadSheetDiskFormat< L, Compare >::write(
                             const std::string& name,
                             ...
                             bool /*ascii*/ ) const
{
  ...
  try
  {
    ...
    while ( l != le )
    {
      ...
      // getting label and fiber measure map
      L label = l->first;
      const std::map< int32_t, std::map< std::string, gkg::Matrix > >&
        fiberMeasureMap = l->second;

      // looping over fibers
      std::map< int32_t, std::map< std::string, gkg::Matrix > >::const_iterator
        f = fiberMeasureMap.begin(),
        fe = fiberMeasureMap.end();
      while ( f != fe )
      {

        // getting fiber index and measure map
        int32_t fiberIndex = f->first;
  ...
  GKG_CATCH( "template < class L, class Compare > "
             ...
             "name, gkg::FiberMeasureSpreadSheet< L, Compare >& object, "
             "bool ascii ) const" );
...
template < class L, class Compare >
inline
bool gkg::FiberMeasureSpreadSheetDiskFormat< L, Compare >::hasReader() const
{
...
template < class L, class Compare >
inline
bool gkg::FiberMeasureSpreadSheetDiskFormat< L, Compare >::hasWriter() const
{
...
inline
std::string 
gkg::FiberMeasureSpreadSheetDiskFormat< L, Compare >::getHeaderExtension() const
{

  return std::string( ".fibermeasure_spreadsheet" ) + 
