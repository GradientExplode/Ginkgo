#include <gkg-dmri-odf-functor/OdfFunctorMDScaledMaxNormalizedMesh.h>
...
gkg::OdfFunctorMDScaledMaxNormalizedMesh::
                                      OdfFunctorMDScaledMaxNormalizedMesh()
                                             : gkg::OdfFunctor<
...
std::string gkg::OdfFunctorMDScaledMaxNormalizedMesh::getName() const
{

  return "md_scaled_max_normalized_mesh";

}


bool 
gkg::OdfFunctorMDScaledMaxNormalizedMesh::isStandardBasisRequired() const
{
...
void gkg::OdfFunctorMDScaledMaxNormalizedMesh::process(
                             ...
                             bool verbose ) const
{
  ...
  try
  {
    ...
    std::vector< double > specificScalarParameters( 6 );
    ...
    while ( s != se )
    {
      ...
      specificScalarParameters[ 5 ] = normalizedMeanDiffusivity( *s - offset );
...
void gkg::OdfFunctorMDScaledMaxNormalizedMesh::processItem(
                 ...
                 const std::vector< std::string >& /*stringParameters*/ ) const
{
  ...
  try
  {
    ...
    // obtaining MD
    float normalizedMeanDiffusivity = ( float )scalarParameters[ 5 ];
    ...
    gkg::Rescaler< std::vector< float >, std::vector< float > >
      rescaler( 0.0, maximum,
                0.0, meshScale * ( 1.0 - normalizedMeanDiffusivity ) );
    rescaler.rescale( item, item );

  }
  GKG_CATCH( "void gkg::OdfFunctorMDScaledMaxNormalizedMesh::processItem( "
             "std::vector< float >& item, "
             "const gkg::OrientationDistributionFunction& odf, "
             "const std::vector< double >& scalarParameters, "
             "const std::vector< std::string >& /*stringParameters*/ ) "
...
static bool register_gkg_OdfFunctorMDScaledMaxNormalizedMesh 
                                                       __attribute__((unused)) =
gkg::OdfFunctorFactory< gkg::MeshMap< int32_t, float, 3U > ,
                        std::vector< float > >::getInstance().
                                                           registerOdfFunctor(
                 gkg::OdfFunctorMDScaledMaxNormalizedMesh::getInstance() );
