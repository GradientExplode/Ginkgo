#include <gkg-dmri-odf-functor/OdfFunctorNormalizedMeanDwi.h>
...
gkg::OdfFunctorNormalizedMeanDwi::OdfFunctorNormalizedMeanDwi()
...
std::string gkg::OdfFunctorNormalizedMeanDwi::getName() const
{

  return "normalized_mean_dwi";

}


bool gkg::OdfFunctorNormalizedMeanDwi::isStandardBasisRequired() const
{
...
void gkg::OdfFunctorNormalizedMeanDwi::process(
                             ...
                             bool verbose ) const
{
  ...
  try
  {
    ...
    gkg::OdfFunctorFactory< gkg::Volume< float >, float >::getInstance().
          getOdfFunctor( "mean_dwi" ).process( output,
                                               odfCartesianField,
                                               scalarParameters,
                                               stringParameters,
                                               false );
    ...
    // processing MeanDwi cumulated histogram
    ...
    // processing MeanDwi rescaler boundaries
    ...
    float meanDwiLowerBoundary = lowerBoundary;
    float meanDwiUpperBoundary = lowerBoundary + ( float )level * 
                             ( upperBoundary - lowerBoundary ) /
                             ( float )( CUMULATED_HISTOGRAM_BIN_COUNT - 1 );

    gkg::Rescaler< gkg::Volume< float >,
                   gkg::Volume< float > > rescaler( meanDwiLowerBoundary,
                                                    meanDwiUpperBoundary,
                                                    0.0,
                                                    1.0 );
  ...
  GKG_CATCH( "void gkg::OdfFunctorNormalizedMeanDwi::process( "
             "gkg::Volume< float >& output, "
             "gkg::OdfCartesianField& odfCartesianField, "
             "const std::vector< double >& scalarParameters, "
             "const std::vector< std::string >& stringParameters, "
...
static bool register_gkg_OdfFunctorNormalizedMeanDwi __attribute__((unused)) =
gkg::OdfFunctorFactory< gkg::Volume< float >, float >::getInstance().
          registerOdfFunctor( gkg::OdfFunctorNormalizedMeanDwi::getInstance() );
