...
const gkg::Vector& 
gkg::AnalyticalQBallOdf::getDwSphericalHarmonicsCoefficients() const
{
  ...
  GKG_CATCH( "const gkg::Vector& "
             "gkg::AnalyticalQBallOdf::getDwSphericalHarmonicsCoefficients() "
	     "const" );

}


void gkg::AnalyticalQBallOdf::getReconstructionMatrix(
                                ...
                                gkg::Matrix& Bplus )
{
  ...
  try
  {
    ...
    if ( laplacianSharpening > 0 )
    {
      ...
      reconstructionMatrix = P.getComposition( Sharpening ).getComposition(
                                                                        Bplus );
    ...
    else
    {
      ...
      // building reconstruction matrix PBplus
      reconstructionMatrix = P.getComposition( Bplus );

    }

  }
  GKG_CATCH( "void gkg::AnalyticalQBallOdf::getReconstructionMatrix( "
             "const gkg::HeaderedObject& headeredObject, "
...
template < class S >
void gkg::AnalyticalQBallOdf::reconstruct(
                             const gkg::OrientationSet* orientationSet,
                             const gkg::Volume< float >& t2,
                             const gkg::Volume< float >& dw,
                             const gkg::Vector3d< S >& site,
    ...
    if ( orientationSet )
    {
      ...
      else
      {
        ...
        for ( s = 0; s < shCoefficientCount; s++ )
        {
          ...
          _values[ s ] = ( float )odfSphericalHarmonicsCoefficients( s );
