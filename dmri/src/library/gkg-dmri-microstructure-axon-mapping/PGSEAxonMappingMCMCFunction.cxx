...
gkg::PGSEAxonMappingMCMCFunction::PGSEAxonMappingMCMCFunction( 
                                                        ...
                                                        noiseStandardDeviation )
{
  ...
  GKG_CATCH( "gkg::PGSEAxonMappingMCMCFunction::PGSEAxonMappingMCMCFunction( "
             "gkg::RCPointer< gkg::Volume< float > > dw, "
             "const gkg::OrientationSet& outputOrientationSet, "
             "double noiseStandardDeviation )" );
...
void gkg::PGSEAxonMappingMCMCFunction::getValuesAt( 
                                                  ...
                                                  gkg::Vector& values ) const
{
  ...
  try
  {
    ...
    const double& isotropicFraction =
      parameters( PGSE_IsotropicFraction );
    const double& isotropicDiffusivity = 
      parameters( PGSE_IsotropicDiffusivity );
    const double& extraPerpendicularDiffusivity = 
      parameters( PGSE_ExtraPerpendicularDiffusivity );
    const double& stationaryFraction =
      parameters( PGSE_StationaryFraction );
    const double& intracellularFraction =
      parameters( PGSE_IntracellularFraction );
    const double& kappa =
      parameters( PGSE_Kappa );
    const double& parallelDiffusivity =
      parameters( PGSE_ParallelDiffusivity );
    const double& axonDiameter =
      parameters( PGSE_AxonDiameter );
    gkg::Vector3d< double > principalOrientation(
                                  parameters( PGSE_PrincipalOrientation ),
                                  parameters( PGSE_PrincipalOrientation + 1 ),
                                  parameters( PGSE_PrincipalOrientation + 2 ) );
    ...
    gkg::PGSEIntracellularContributionCache& 
      pgseIntracellularContributionCache =
        gkg::PGSEIntracellularContributionCache::getInstance();

    std::vector< double >::const_iterator 
      bPtr = pgseIntracellularContributionCache.getBValues().begin();
    ...
    for ( i = 0; i < this->_inputOrientationCount; i++, inputPtr += 6, bPtr++ )
    {
      ...
      intracellularContribution = pgseIntracellularContributionCache.
  ...
  GKG_CATCH( "void gkg::PGSEAxonMappingMCMCFunction::getValuesAt( "
             "const gkg::Vector& parameters, "
             "gkg::Vector& values ) const" );
...
void gkg::PGSEAxonMappingMCMCFunction::getModelAt(
             const gkg::Vector& parameters,
             ...
             std::vector< float >& coordinates ) const
{
  ...
  try
  {
    ...
    gkg::Vector3d< float > principalOrientation(
                         ( float )parameters( PGSE_PrincipalOrientation ),
                         ( float )parameters( PGSE_PrincipalOrientation + 1 ),
                         ( float )parameters( PGSE_PrincipalOrientation + 2 ) );
    ...
    for ( b = 0; b < bValueCount; b++ )
    {
      ...
      double gradientMagnitude = double( gradientCharacteristics[ b ][ 0 ] );
      double littleDelta = double( gradientCharacteristics[ b ][ 1 ] );
      double bigDelta = double( gradientCharacteristics[ b ][ 2 ] );
      double riseTime = double( gradientCharacteristics[ b ][ 3 ] );
  ...
  GKG_CATCH( "void gkg::PGSEAxonMappingMCMCFunction::getModelAt( "
             "const gkg::Vector& parameters, "
             "const std::vector< float >& multipleShellBValues, "
...
gkg::PGSEAxonMappingMCMCFunction::getType() const
{

  try
  {

    return gkg::OrientationDistributionFunction::PGSEAxonMapping;
...
int32_t gkg::PGSEAxonMappingMCMCFunction::getParameterCount() const
{
  ...
  GKG_CATCH( "int32_t "
             "gkg::PGSEAxonMappingMCMCFunction::getParameterCount() const" );

}


void gkg::PGSEAxonMappingMCMCFunction::getApparentExtracellularTensor(
                            ...
                            double* apparentExtracellularTensor ) const
{
  ...
  GKG_CATCH( "void gkg::PGSEAxonMappingMCMCFunction::"
