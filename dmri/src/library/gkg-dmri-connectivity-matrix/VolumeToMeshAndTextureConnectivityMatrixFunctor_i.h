#ifndef _gkg_dmri_connectivity_matrix_VolumeToMeshAndTextureConnectivityMatrixFunctor_i_h_
#define _gkg_dmri_connectivity_matrix_VolumeToMeshAndTextureConnectivityMatrixFunctor_i_h_

#include <gkg-dmri-connectivity-matrix/VolumeToMeshAndTextureConnectivityMatrixFunctor.h>
...
//
// class VolumeToMeshAndTextureLoopContext< L, Compare >
//

template < class L, class Compare >
inline
gkg::VolumeToMeshAndTextureLoopContext< L, Compare >
                                            ::VolumeToMeshAndTextureLoopContext(
     gkg::ConnectivityMatrixGauge& gauge,
     const gkg::VolumeToMeshAndTextureConnectivityMatrixFunctor< L, Compare >& 
     ...
     const gkg::TextureMap< int16_t >&  rois2Texture,
...
template < class L, class Compare >
inline
void gkg::VolumeToMeshAndTextureLoopContext< L, Compare >::doIt( 
                                                             int32_t startIndex,
                                                             int32_t count )
{
  ...
  try
  {
    ...
    for ( s = startIndex, g = 0; s < stopIndex; g++, s++ )
    {
      ...
      std::map< L, std::list< gkg::Polygon< 3U > > > polygonMap;
      std::map< L, std::list< gkg::Vector3d< float > > > projectionPointMap;
      std::map< L, int32_t > correspondingRois1Label;
...
inline
int32_t 
gkg::VolumeToMeshAndTextureLoopContext< L, Compare >::resampleFiber(
...
inline
void 
gkg::VolumeToMeshAndTextureLoopContext< L, Compare >::findLabels(
      ...
      if ( *l == LABEL_FOR_BACKGROUND )
      {
        ...
        if ( _rois2BoundingBox.contains( pointInRois2RealFrame ) )
        {
          ...
          if ( _rois2Mask( voxelInRois2VoxelFrame ) )
          {
            ...
            if ( _volumeToMeshAndTextureConnectivityMatrixFunctor.
...

            {
              ...
              if ( *d <= _volumeToMeshAndTextureConnectivityMatrixFunctor.
...
template < class L, class Compare >
inline
int32_t gkg::VolumeToMeshAndTextureLoopContext< L, Compare >::getRange(
...
template < class L, class Compare >
inline
void gkg::VolumeToMeshAndTextureLoopContext< L, Compare >
   ...
   std::map< L, int32_t >& correspondingRois1Label ) const
{
    ...
    for ( ; r1 < rangeCount; r1++, r2++ )
    {
           ...
           ( label2 > LABEL_FOR_BACKGROUND ) )
      {
        ...
        L newLabel = getLabel( label2,
                               polygons[ indexOfMinimumDistance ],
        ...
        labelledFibers[ newLabel ].push_back(
          ...
          _volumeToMeshAndTextureConnectivityMatrixFunctor.
                                                 _outputFiberResamplingStep ) );

        polygonMap[ newLabel ].push_back( polygons[ indexOfMinimumDistance ] );
        projectionPointMap[ newLabel ].push_back( 
                                   projectionPoints[ indexOfMinimumDistance ] );
	
        correspondingRois1Label[ newLabel ] = label2;
                ...
                ( label2 == LABEL_FOR_MESH ) )
      {
        ...
        projectionPointMap[ newLabel ].push_back( 
                                   projectionPoints[ indexOfMinimumDistance ] );
        correspondingRois1Label[ newLabel ] = label1;
  ...
  GKG_CATCH( "template < class L, class Compare > "
             ...
             "labelledFibers, "
             "std::map< L, std::list< gkg::Polygon< 3U > > >& polygonMap, "
             "std::map< L, std::list< gkg::Vector3d< float > > >& "
             "projectionPointMap,"
             "std::map< L, int32_t >& correspondingRois1Label ) const" );
...
template < class L, class Compare >
inline
void gkg::VolumeToMeshAndTextureLoopContext< L, Compare >::fillOutputs(
  const std::map< L, std::list< gkg::LightCurve3d< float > > >&  labelledFibers,
  const std::map< L, std::list< gkg::Polygon< 3U > > >&  polygonMap,
  const std::map< L, std::list< gkg::Vector3d< float > > >& projectionPointMap,
  const std::map< L, int32_t >& correspondingRois1Label ) 
{
  ...
  try
  {
    ...
    typename std::map< L, std::list< gkg::Polygon< 3U > > >::const_iterator
      pm = polygonMap.begin(),
      pme = polygonMap.end();
      
    typename std::map< L, std::list< gkg::Vector3d< float > > >::const_iterator
    ...
    typename std::map< L, int32_t >::const_iterator
    ...
    while( pm != pme )
    {
      ...
      while( po != poe )
      {
        ...
        const int16_t& label1 = _rois2Texture.getTexture( index1 );
        const int16_t& label2 = _rois2Texture.getTexture( index2 );
        const int16_t& label3 = _rois2Texture.getTexture( index3 );
        ...
        if ( !_connectivityMatrix.hasElement( label, label1 ) )
        {

          lock();
          _connectivityMatrix( label, label1 ) = ( double )weight1;
          unlock();
        ...
        if ( !_connectivityMatrix.hasElement( label, label2 ) )
        {

          lock();
          _connectivityMatrix( label, label2 ) = ( double )weight2;
          unlock();
        ...
        if ( !_connectivityMatrix.hasElement( label, label3 ) )
        {

          lock();
          _connectivityMatrix( label, label3 ) = ( double )weight3;
          unlock();
        ...
        if ( !_averageLengthMatrix.isNull() )
        {
          ...
          if ( !_averageLengthMatrix->hasElement( label, label1 ) )
          {

            lock();
            ( *_averageLengthMatrix )( label, label1 ) =  fiberLength * weight1;
            unlock();
          ...
          if ( !_averageLengthMatrix->hasElement( label, label2 ) )
          {

            lock();
            ( *_averageLengthMatrix )( label, label2 ) = fiberLength * weight2;
            unlock();
          ...
          if ( !_averageLengthMatrix->hasElement( label, label3 ) )
          {

            lock();
            ( *_averageLengthMatrix )( label, label3 ) = fiberLength * weight3;
            unlock();
  ...
  GKG_CATCH( "template < class L, class Compare > "
             ...
             "labelledFibers,"
             "const std::map< L, std::list< gkg::Polygon< 3U > > >&"
             "polygonMap,"
             "const std::map< L, std::list< gkg::Vector3d< float > > >&"
             "projectionPointMap,"
             "const std::map< L, int32_t >& correspondingRois1Label )" ); 
...
template < class L, class Compare >
inline
L gkg::VolumeToMeshAndTextureLoopContext< L, Compare >::getLabel( 
                                  int32_t rois1Label,
                                  ...
                                  gkg::Vector3d< float > projectionPoint ) const
{
  ...
  GKG_CATCH( "template < class L, class Compare > "
             "inline "
             "L gkg::VolumeToMeshAndTextureLoopContext< L, Compare >::"
             "getLabel( "
             "int32_t rois1Label,"
             "gkg::Polygon< 3U> polygon,"
             "gkg::Vector3d< float > projectionPoint ) const" );
...
inline
std::string 
VolumeToMeshAndTextureLoopContext< std::string, std::less< std::string > >
                                                                     ::getLabel(
                                  int32_t rois1Label,
                                  gkg::Polygon< 3U> polygon,
                                  gkg::Vector3d< float > projectionPoint ) const
{
  ...
  try
  {
    ...
    std::string label;
    if( ( polygon.indices[ 0 ] == polygon.indices[ 1 ] ) &&
        ( polygon.indices[ 1 ] == polygon.indices[ 2 ] ) )
    {
    
      label = gkg::StringConverter::toString( rois1Label ) + '_' +
              gkg::StringConverter::toString( _rois2Texture.getTexture(
                                                       polygon.indices[ 0 ] ) );
    ...
    else
    {
      ...
      _evolvingMesh->getPartialTriangleAreaWeighting( polygon,
                                                      projectionPoint,
                                                      weight1,
                                                      weight2,
                                                      weight3 );
      if( weight1 > maximumWeight )
      {
      
        maximumWeight = weight1;
        rois2Label = _rois2Texture.getTexture( polygon.indices[ 0 ] ); 
      
      }
      if( weight2 > maximumWeight )
      {
      
        maximumWeight = weight2;
        rois2Label = _rois2Texture.getTexture( polygon.indices[ 1 ] ); 
      
      }
      if( weight3 > maximumWeight )
      {
      
        maximumWeight = weight3;
        rois2Label = _rois2Texture.getTexture( polygon.indices[ 2 ] ); 
      
      }

      label = gkg::StringConverter::toString( rois1Label ) + '_' +
              gkg::StringConverter::toString( rois2Label );
  ...
  GKG_CATCH( "inline "
             "std::string  "
             "VolumeToMeshAndTextureLoopContext< std::string, "
...
inline
int16_t 
VolumeToMeshAndTextureLoopContext< int16_t,std::less< int16_t > >::getLabel(
                                  int32_t rois1Label,
                                  gkg::Polygon< 3U> polygon,
                                  gkg::Vector3d< float > projectionPoint ) const
{
  ...
  try
  {
        ...
        ( polygon.indices[ 1 ] == polygon.indices[ 2 ] ) )
    {
      ...
      label = ( int16_t )rois1Label + 
              _volumeToMeshAndTextureConnectivityMatrixFunctor._offset * 
                               _rois2Texture.getTexture( polygon.indices[ 0 ] );
  ...
  GKG_CATCH( "inline "
             "int16_t  "
             "VolumeToMeshAndTextureLoopContext< int16_t, "
...
//
// class VolumeToMeshAndTextureConnectivityMatrixFunctor< L, Compare >
//

template < class L, class Compare >
inline
gkg::VolumeToMeshAndTextureConnectivityMatrixFunctor< L, Compare >::
                                VolumeToMeshAndTextureConnectivityMatrixFunctor(
                                ...
                                : gkg::ConnectivityMatrixFunctor< L, Compare >()
{
  ...
  try
  {
    ...
    else
    {
      ...
      if ( scalarParameters.size() != 11U )
      {

        throw std::runtime_error( "11 scalar parameters required" );
      ...
      _offset = ( int16_t )( scalarParameters[ 10 ] + 0.5 );
...
template < class L, class Compare >
inline
void gkg::VolumeToMeshAndTextureConnectivityMatrixFunctor< L, Compare >::apply(
                ...
                bool verbose ) const
{
  ...
  try
  {
    ...
    if ( fileNameListOfROIs2.size() != 2U )
    {

      throw std::runtime_error( "input mesh and texture filenames required" );
    ...
    ////////////////////////////////////////////////////////////////////////////
    // reading input texture for ROIs2 
    ////////////////////////////////////////////////////////////////////////////

    std::string textureFileNameROIs2 = fileNameListOfROIs2.back();
    if ( verbose )
    {

      std::cout << "reading ROIs2 texture '" << textureFileNameROIs2 << "'"
                << std::flush;

    }

    gkg::TextureMap< int16_t > rois2Texture;
    gkg::Reader::getInstance().read( textureFileNameROIs2, rois2Texture );

    if ( rois2Texture.getCount() != vertexCount )
    {

      throw std::runtime_error( "texture and mesh in ROI2 must have "
    ...
    if ( verbose )
    {
      ...
      std::cout << gkg::Eraser( 24 + ( int32_t )textureFileNameROIs2.size() )
                << std::flush;
    ...
    int32_t maximumLabelRois1 = ( int32_t )*rois1Labels.rbegin();
    if ( verbose )
    {
    ...
    ////////////////////////////////////////////////////////////////////////////
    // computing the number of ROIs2 labels
    ////////////////////////////////////////////////////////////////////////////

    if ( verbose )
    {

     std::cout << "computing labels of ROIs2" << std::flush;

    }
    std::set< int16_t > rois2Labels;
    
    gkg::TextureMap< int16_t >::const_iterator
      t = rois2Texture.begin(),
      te = rois2Texture.end();
    ...
    while ( t != te )
    {
      ...
      if ( t->second > 0 )
      {
        ...
        rois2Labels.insert( t->second );

      }
      ++ t;

    }
    int32_t maximumLabelRois2 = ( int32_t )*rois2Labels.rbegin();
    if ( verbose )
    {
    ...
    gkg::SparseMatrix connectivityMatrix( maximumLabelRois1 + 1, 
                                          maximumLabelRois2 + 1 );
         ...
         setOfFunctorNames.end() )
    {
      ...
      averageLengthMatrix.reset( new gkg::SparseMatrix( 
                                                      maximumLabelRois1 + 1,
                                                      maximumLabelRois2 + 1 ) );
    ...
    gkg::RCPointer< gkg::BundleMap< L, Compare > > roiToMeshAndTextureBundleMap;
    if ( setOfFunctorNames.find( "roi-to-mesh-and-texture-bundle-map" ) !=
         setOfFunctorNames.end() )
    {

      roiToMeshAndTextureBundleMap.reset( new gkg::BundleMap< L, Compare > );
         ...
         setOfFunctorNames.end() )
    {
      ...
      for ( ; r1 != r1e; r1++ )
      {
        ...
        for ( s2 = 0; s2 < vertexCount; s2++ )
        {
          ...
          label2 = rois2Texture.getTexture( s2 );
          if ( connectivityMatrix.hasElement( *r1, ( int32_t )label2 ) )
          {
 
            connectivities[ s2 ] = connectivityMatrix( *r1, ( int32_t )label2 );
         ...
         setOfFunctorNames.end() )
    {
      ...
      if ( verbose )
      {
       ...
       std::cout << "saving ROIs1 to mesh and texture bundle map" << std::flush;

      }


      gkg::Writer::getInstance().write(
                      outputFileNameMap[ "roi-to-mesh-and-texture-bundle-map" ],
                      *roiToMeshAndTextureBundleMap,
                      ascii,
  ...
  GKG_CATCH( "template < class L, class Compare > "
             "inline "
             "void "
             "gkg::VolumeToMeshAndTextureConnectivityMatrixFunctor< L, "
             "Compare >::"
             "apply( "
             "const std::list< std::string >& fileNameBundleMaps, "
             "const std::list< std::string >& fileNameListOfROIs1, "
                                            ...
                                            Compare >::getScalarParameterHelp()
{
  ...
  try
  {
    ...
    help += ".  <P11>: offset for output label creation when input \n";
    help += ".         bundle map labels are of type int16_t (default=0)\n\n";
                                            ...
                                            Compare >::getFunctorHelp()
{
  ...
  try
  {
    ...
    help += ".  roi-to-mesh-and-texture-bundle-map\n";
