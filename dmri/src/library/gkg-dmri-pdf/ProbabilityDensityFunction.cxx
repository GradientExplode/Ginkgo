...
float gkg::ProbabilityDensityFunction::dot( 
    ...
    if ( _basisType == gkg::ProbabilityDensityFunction::PolarHarmonics )
    {
      ...
      for ( c1 = 0; c1 < phCoefficientCount; c1++ )
      {
        ...
        for ( c2 = 0; c2 < phCoefficientCount; c2++ )
        {
          ...
          dotProduct += this->_values[ 7 + c1 ] *
                        otherValues[ 7 + c2 ] *
                        polarHarmonicsCache.getTmn(
                                           n1n2n3.x,
                                           otherN1n2n3.x,
                                           scalingParameters.x,
        ...
        const gkg::Matrix& Bplus ) const
{
  ...
  try
  {
    ...
    std::vector< float > displacementProbabilities =
                                           this->getDisplacementProbabilities();
    odf.setValues( Bplus.getComposition(
                       gkg::Vector( displacementProbabilities ).normalize() ) );
