#include <gkg-dmri-plugin-functors/DwiNoiseFilter/DwiNoiseFilterCommand.h>
...
gkg::DwiNoiseFilterCommand::DwiNoiseFilterCommand( int32_t argc,
                                                   char* argv[],
                                                   bool loadPlugin,
                                                   bool removeFirst )
                           : gkg::Command( argc, argv, loadPlugin, removeFirst )
{
  ...
  GKG_CATCH( "gkg::DwiNoiseFilterCommand::DwiNoiseFilterCommand( "
...
gkg::DwiNoiseFilterCommand::DwiNoiseFilterCommand(
...
std::string gkg::DwiNoiseFilterCommand::getStaticName()
{

  try
  {

    return "DwiNoiseFilter";

  }
  GKG_CATCH( "std::string gkg::DwiNoiseFilterCommand::getStaticName()" );

}


void gkg::DwiNoiseFilterCommand::parse()
{
  ...
  GKG_CATCH_FUNCTOR( "void gkg::DwiNoiseFilterCommand::parse()" );

}


void gkg::DwiNoiseFilterCommand::execute(
                            const std::string& fileNameT2,
                            const std::string& fileNameDW,
                            const std::string& fileNameMaskOfSignal,
                            const std::string& fileNameMaskOfNoise,
...
*/
    ...
    if ( verbose )
    {
      ...
      std::cout << "building Kalman structure : " << std::flush;

    }

    gkg::KalmanFilter kalmanFilter( B );
    ...
    float normalizedDenoisedDW = 0.0;
            ...
            if ( maskOfSignal( site ) && ( averageT2( site ) > 0 ) )
            {
              ...
              if ( denominator > EPSILON )
              {
                ...
                if ( denoisedDWSquare > 0.0 )
                {
                  ...
                  normalizedDenoisedDW = std::sqrt( denoisedDWSquare ) / 
                                         denoisedT2( site );
                ...
                else
                {
                  ...
                  normalizedDenoisedDW = value / denoisedT2( site );
...
*/
  ...
  GKG_CATCH( "void gkg::DwiNoiseFilterCommand::execute( "
             "const std::string& fileNameT2, "
             "const std::string& fileNameDW, "
             "const std::string& fileNameMaskOfSignal, "
             "const std::string& fileNameMaskOfNoise, "
